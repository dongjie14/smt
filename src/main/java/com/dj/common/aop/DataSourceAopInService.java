...
import com.dj.common.dbconfig.DataSourceType;
...
/*	@Before("execution(* com.fei.springboot.service..*.find*(..)) "
	...
	@Before("execution(* com.dj.*.service..*.*(..)) "
			+ " and @annotation(com.dj.common.annotation.ReadDataSource) ")
	public void setReadDataSourceType() {
		//如果已经开启写事务了，那之后的所有读都从写库读
		if(!DataSourceType.write.getType().equals(DataSourceContextHolder.getReadOrWrite())){
			DataSourceContextHolder.setRead();
	...
	@Before("execution(* com.dj.*.service..*.*(..)) "
			+ " and @annotation(com.dj.common.annotation.WriteDataSource) ")
	public void setWriteDataSourceType() {
	    DataSourceContextHolder.setWrite();
	}
